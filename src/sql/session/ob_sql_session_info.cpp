    ...
    uint64_t tenant_id)
{
  ...
  if (OB_FAIL(ObBasicSessionInfo::init(sessid, proxy_sessid, bucket_allocator, tz_info))) {
    ...
    LOG_WARN("create pyudf funcache map failed", K(ret));
             ...
             OB_FAIL(pyudf_redundent_funcache_map_.create(hash::cal_next_prime(32),
                                          ObModIds::OB_HASH_BUCKET,
                                          ObModIds::OB_HASH_NODE))){
    LOG_WARN("create pyudf funcache map failed", K(ret));
  } else if(!is_acquire_from_pool() &&
             OB_FAIL(history_pyudf_map_.create(hash::cal_next_prime(32),
                                          ObModIds::OB_HASH_BUCKET,
                                          ObModIds::OB_HASH_NODE))){
    LOG_WARN("create pyudf funcache map failed", K(ret));
  } else if(!is_acquire_from_pool() &&
             OB_FAIL(merged_udf_pre_map_.create(hash::cal_next_prime(32),
                                          ObModIds::OB_HASH_BUCKET,
                                          ObModIds::OB_HASH_NODE))){
    LOG_WARN("create pyudf funcache map failed", K(ret));
  } else if(!is_acquire_from_pool() &&
             OB_FAIL(pyudf_str_funcache_map_.create(hash::cal_next_prime(32),
                                          ObModIds::OB_HASH_BUCKET,
                                          ObModIds::OB_HASH_NODE))){
    LOG_WARN("create pyudf funcache map failed", K(ret));
  } else{
