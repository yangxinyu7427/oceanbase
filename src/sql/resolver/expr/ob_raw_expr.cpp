...
int ObRawExprFactory::create_raw_expr<ObSysFunRawExpr>(ObItemType expr_type, ObSysFunRawExpr *&raw_expr)
{
  ...
  switch (expr_type) {
    ...
    GENERATE_CASE(T_FUN_PYTHON_UDF, ObPythonUdfRawExpr);
...
int ObPythonUdfRawExpr::set_udf_meta_has_no_output_model_path(){
  int ret = OB_SUCCESS;
  udf_meta_.has_new_output_model_path_=false;
  return ret;
}

int ObPythonUdfRawExpr::set_udf_meta_can_be_used_model_path(ObString can_be_used_model_path){
  int ret = OB_SUCCESS;
  udf_meta_.can_be_used_model_path_=can_be_used_model_path;
  return ret;
}
                                          ...
                                          ExplainType type) const
{
  int ret = OB_SUCCESS;
  int64_t i = 0;
  if (OB_FAIL(BUF_PRINTF("T_FUN_PYTHON_UDF{%.*s}(", udf_meta_.name_.length(), udf_meta_.name_.ptr()))) {
      LOG_WARN("fail to BUF_PRINTF", K(ret));
                                      ...
                                      ObRawExpr *&dest)
{
  ...
  case ObRawExpr::EXPR_SYS_FUNC: {
    if (T_FUN_SYS_SEQ_NEXTVAL == expr_type) {
      ...
      } else if (T_FUN_PYTHON_UDF == expr_type) {
  ...
  case ObRawExpr::EXPR_PYTHON_UDF: {
    ObPythonUdfRawExpr *dest_pyudf_expr = NULL;
    if (OB_FAIL(create_raw_expr(expr_type, dest_pyudf_expr))
        || OB_ISNULL(dest_pyudf_expr)) {
      LOG_WARN("failed to allocate raw expr", K(dest_pyudf_expr), K(ret));
    } else {
      dest = dest_pyudf_expr;
    }
    break;
  }
