...
void ObServerSchemaService::AllSchemaKeys::reset()
{
  ...
  new_udf_keys_.clear();
  del_udf_keys_.clear();
  new_model_keys_.clear();
  del_model_keys_.clear();
  new_udt_keys_.clear();
  del_udt_keys_.clear();
...
int ObServerSchemaService::AllSchemaKeys::create(int64_t bucket_size)
{
  ...
  } else if (OB_FAIL(new_model_keys_.create(bucket_size))) {
    LOG_WARN("failed to create new_model_ids hashset", K(bucket_size), K(ret));
  } else if (OB_FAIL(del_model_keys_.create(bucket_size))) {
    LOG_WARN("failed to create del_model_ids hashset", K(bucket_size), K(ret));
    ...
    const bool new_flag)
{
  ...
  if (OB_INVALID_ID == tenant_id) {
    ...
    LOG_WARN("fail to del udf operation", KR(ret), K(tenant_id));
             ...
             new_flag ? schema_keys.new_model_keys_ : schema_keys.del_model_keys_))) {
    LOG_WARN("fail to del udf operation", KR(ret), K(tenant_id));
  } else if (OB_FAIL(del_operation(tenant_id,
    ...
    ObSchemaMgr &schema_mgr)
{
  ...
  } else if (OB_FAIL(apply_model_schema_to_cache(
             tenant_id, all_keys, simple_incre_schemas, schema_mgr.python_udf_mgr_))) {
    LOG_WARN("fail to apply model schema to cache", KR(ret), K(tenant_id));
...
APPLY_SCHEMA_TO_CACHE_IMPL(ObPythonUDFMgr, model, ObSimpleModelSchema, ModelKeys);
